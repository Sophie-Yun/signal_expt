...
//mpotter
function calculate_Euclidean_Steps(obj, targetX, targetY, player){
    var startX;
    var startY;
    if(player == "signaler"){
        startX = obj.signalerLocation[0];
        startY = obj.signalerLocation[1];
    }
    else{
        startX = obj.receiverLocation[0];
...
//mpotter
function calculate_Euclidean_Steps_IntermittentCoord(obj, targetX, targetY, player, totalDistance){
    var startX;
    var startY;
    if(player == "signaler"){
    ...
    //totalDistance = euclideanDistance([startX,startY],[targetX,targetY]);
    //console.log("total Distance: ", totalDistance)
    //totalSteps = Math.round(totalDistance);
...
//mpotter
function convertEachStepToGridMovement(eachStep){
    //xCoord in obj.signalerlocation or receiverlocation corresponds to vertical height.
    //Negative moves up, positive moves down
    //So, we need to take eachStep[1], negate it, then insert it in xcoord spot

    //yCoord in obj.location (for sig or rec) corresponds to horizontal 
    //Negative moves left, positive moves Right
    //So, we need to take eachStep[0], and insert it into ycoord spot
...
//mpotter
function RECEIVER_MOVES_EUCLIDEAN(obj, correctedStep, eachCoordStep){
    RECEIVER_FIRST_MOVE(obj);
    REMOVE_PREVIOUS(obj.receiverLocation);
    obj.receiverLocation = [obj.receiverLocation[0] + correctedStep[0], obj.receiverLocation[1]+correctedStep[1]];//
    RECORD_RECEIVER_PATH(obj);
    NEW_RECEIVER_POSITION(obj.receiverLocation);
...
function UPDATE_GAME_GRID_EUCLIDEAN(obj, eachStep, eachCoordStep, player) {
    ...
    if (player == "signaler"){
        SIGNALER_MOVES_EUCLIDEAN(obj, convertedMovement, eachCoordStep);
    }
    else{
        RECEIVER_MOVES_EUCLIDEAN(obj, convertedMovement, eachCoordStep);
    }
}
